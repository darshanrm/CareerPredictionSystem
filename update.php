...
{
$sn++;
header("location:account.php?q=quiz&step=2&eid=$eid&n=$sn&t=$total&d=$dept&tag=$tag")or die('Error152');
}
...
header("location:account.php?q=result&eid=$eid&d=$dept&tag=$tag");
}
else
{
header("location:account.php?q=result&eid=$eid&d=$dept&tag=$tag");
...
$sun=$sun-$s;
$q=mysqli_query($con,"UPDATE `rank` SET `score`=$sun ,time=NOW() WHERE email= '$email'")or die('Error174');
header("location:account.php?q=quiz&step=2&eid=$eid&n=1&t=$t&d=$dept&tag=$tag");
}
